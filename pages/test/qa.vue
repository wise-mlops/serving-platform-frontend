                    ...
                    <VaList>
                        <VaListItem class="input-set-row">
                            ...
                            <VaInput v-model="batchSize" class="batch-size ml-2" placeholder="모델 병렬 처리 단위"
                                label="batch size" type="number" :min="1"
...
// 입력 값
const model = ref<string>("t5-small-fid");
...
const isFormValid = computed(() => {
    ...
    const questionLenth = question.value.trim().length;
    const contextLength = contextList.value.length;
    return isGreaterThanZero && questionLenth > 0 && contextLength > 0;
})
...
const isEditorValid = computed(() => {
    ...
    try {
        ...
        const mustKeys1 = ['model', 'batch_size', 'message'];
        mustKeys1.forEach(k => {
            if (!(keys.includes(k))) {
                throw new Error;
            }
        });
        const roleKeys = json.message.map((m: Message) => m.role);
...
/**
 * Editor 내에서 입력한 값이 있을 때, Form으로 작성 모드를 바꾸게 되면 입력한 값이 사라집니다.
 */
...
const getResult = async () => {
    try {
        ...
        const response = await restAPI.post(`/kserve/kubeflow-user-example-com/${name.value}/infer?model_format=T5`, body.message);
        if (response) {
